...
async function loadAll() {
  const d = await chrome.storage.local.get(['keywords','settings','accounts','publishedKeywords','writeHistory']);
  S.keywords          = d.keywords          || [];
  S.settings          = d.settings          || {};
  S.accounts          = d.accounts          || [];
  S.publishedKeywords = d.publishedKeywords || [];
  S.writeHistory      = d.writeHistory      || {};
}
  ...
  try {
    const res = await chrome.runtime.sendMessage({
      ...
      options: {
        ...
        minRise: parseInt(S.settings.sMinRise || 0),
    ...
    if (res.success && res.keywords.length > 0) {
      ...
      renderTable();
      updateHeader();
      setStatus(`✅ ${res.keywords.length}개 수집 완료 (신규 ${newCount}개).`);
    } else {
      setStatus('⚠️ 키워드를 가져오지 못했습니다. 네이버 로그인 상태를 확인하세요.');
    }
  } catch(e) {
...
async function analyzeAll() {
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    try {
      const r = await chrome.runtime.sendMessage({ type:'ANALYZE_KEYWORD', keyword:kw.keyword, naverCustomerId:cid, naverAccessLicense:lic, naverSecretKey:sec });
...
async function publishToNaver() {
  ...
  const tab = await chrome.tabs.create({ url:`https://blog.naver.com/${accountId}/postwrite`, active:true });
  await delay(3000);
  try {
    await chrome.tabs.sendMessage(tab.id, { type:'INSERT_POST', title, content });
    markPublished(keyword, title);
    renderTable();
    showToast('블로그 에디터를 열었습니다. 확인 후 발행하세요.');
  } catch(e) { showToast('에디터 입력 오류: '+e.message, true); }
}
...
function saveSettingsUI() {
  ...
  chrome.storage.local.set({ settings:S.settings, accounts:S.accounts });
  updateAnalyzeBtn();
...
async function testNaverApi() {
  ...
  const r = await chrome.runtime.sendMessage({ type:'TEST_NAVER_API', naverCustomerId:cid, naverAccessLicense:lic, naverSecretKey:sec });
...
function addAccount() {
  ...
  chrome.storage.local.set({ accounts:S.accounts });
}
...
function renderAccountList() {
  ...
  c.querySelectorAll('.account-rm').forEach(btn=>{
    btn.addEventListener('click', ()=>{ S.accounts=S.accounts.filter(a=>a!==btn.dataset.id); chrome.storage.local.set({accounts:S.accounts}); renderAccountList(); renderAccountSelect(); });
  });
}
